...
import App from "./App";
import type { Document, AnnotationSet } from "./types";

// KEEP the annotator mock
const MockMarkdownAnnotator = ({ document, annotationSet }: { document: Document | null, annotationSet: AnnotationSet | null }) => {
    if (!document) return null;
    return (
        <div data-testid="annotator-placeholder">
            <h2>Annotator</h2>
            <p>Document: {document.name} ({document.id})</p>
            <p>Annotation Set: {annotationSet ? `${annotationSet.name} (${annotationSet.id})` : 'None'}</p>
        </div>
    );
};
mock.module("./components/markdown-annotator/MarkdownAnnotator", () => ({ default: MockMarkdownAnnotator }));
...
describe("App Component Integration", () => {
    ...
    const uploadFile = async (user: ReturnType<typeof userEvent.setup>, fileName: string, fileContent: string) => {
    ...
    test("adds document via upload, auto-selects, and shows menu/annotator", async () => {
     ...
     test("adds multiple documents, selects one, then selects another", async () => {
        ...
        await uploadFile(user, "Doc A.txt", "Content A");
         await waitFor(() => { expect(screen.getByText("Doc A.txt")).not.toBeNull(); });
        await uploadFile(user, "Doc B.txt", "Content B");
         await waitFor(() => { expect(screen.getByText("Doc B.txt")).not.toBeNull(); });
    ...
    test("adds document, adds annotation set via button, set is selected", async () => {
        ...
        await waitFor(() => { expect(screen.getByText("Annotate Me.md")).not.toBeNull(); });
        await waitFor(() => { expect(screen.getByTestId("document-menu")).not.toBeNull(); }); // Wait for menu
        ...
        await waitFor(() => {
            const combobox = screen.getByRole('combobox', { name: /annotation set/i }) as HTMLSelectElement;
     ...
     test("adds document, adds two sets, selects second via dropdown", async () => {
        const user = userEvent.setup();
        global.prompt = mock(() => "Set Alpha").mockImplementationOnce(() => "Set Beta"); // Mock prompt sequence
        render(<App />);

        // Add doc
        await uploadFile(user, "Doc.txt", "Content");
        await waitFor(() => { expect(screen.getByTestId("document-menu")).not.toBeNull(); });
        ...
        const combobox = screen.getByRole('combobox', { name: /annotation set/i }) as HTMLSelectElement;
        await user.selectOptions(combobox, screen.getByRole('option', { name: 'Set Alpha' }));
    ...
    test("adds document, then removes it using the remove button", async () => {
        ...
        await waitFor(() => { expect(screen.getByText("Delete Me.txt")).not.toBeNull(); });

        // Get the remove button specific to this document
        const docListItem = screen.getByText("Delete Me.txt").closest('li');
        const removeButton = screen.getByRole('button', { name: /remove delete me\.txt/i }); // Use specific label
